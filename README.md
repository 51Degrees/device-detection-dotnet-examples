...
| PreferenceManagement-Web      | Pages carrying the 51Degrees Preference Management Platform and the client script in every arrangement the shared browser tests check, built as plain files so every language can serve the same pages. See its README.     |
...
| PerformanceMultiProperty-Console | Clock-time performance test that reads every available scalar property on each detection, plus an isolated property-read throughput, to measure the on-premise property-read path.                                          |
| PerformanceSweep-Console   | Clock-time performance test that sweeps the number of properties read per detection, showing how end-to-end throughput moves from detection-dominated to read-dominated.                                                          |
